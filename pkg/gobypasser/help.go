...
func Usage() {
	...
	uGeneral := UsageSection{
		...
		ExpectedFlags: []string{"u", "f", "p", "X", "t", "v", "L", "no-color"},
	}
	...
	uFilter := UsageSection{
		...
		Flags:         make([]UsageFlag, 0),
		ExpectedFlags: []string{"hc", "hs", "show-all", "sa"},
	}
	// Catch-all: any flag not claimed above lands here so nothing can silently
	// disappear from the usage output.
	uMisc := UsageSection{
		Name:          "Misc Options",
		Description:   "",
		Flags:         make([]UsageFlag, 0),
	...
	flag.VisitAll(func(f *flag.Flag) {
		...
		if !claimed {
			uMisc.Flags = append(uMisc.Flags, UsageFlag{
				Name:        f.Name,
				Description: f.Usage,
				Default:     f.DefValue,
			})
	...
	sections = append(sections, uMisc)
	...
	fmt.Println("\tgobypasser -u https://www.google.com/ -p admin -verbs")
	fmt.Println("\tgobypasser -u https://www.google.com/ -p admin -verbs -headers -paths")
	fmt.Println("\tgobypasser -u https://www.google.com/ -p admin -X POST -paths -show-all")
	fmt.Println("\tgobypasser -f ./url_list.txt -p admin -verbs -headers -paths -hc 500")
	fmt.Println()
}
...
func VerifyOptions(o *Options) (bool, string) {
	...
	// Normalize the base path to a bare segment; the templates own the slashes.
